...
fn valid_response(res: &str) -> Result<()> {
    ...
    match res {
        ...
        APDU_RSP_FUNCTION_NOT_SUPPORTED => {
            Err(format_err!("{}", "imkey_applet_function_not_supported"))
        }
...
impl HardWalletKeystore {
    ...
    pub fn xpub_apdu(path: &str, verify_key: bool) -> Vec<String> {
        ...
        [parent_path.as_bytes(), path.as_bytes()]
            .iter()
            .map(|bytes| {
                ...
                [header.as_bytes(), bytes.to_vec(), vec![0x00]]
                    .concat()
                    .to_hex()
                    .to_uppercase()
            })
...
mod tests {
    ...
    pub fn xpub_apdu() {
        ...
        let expected = vec![
            "80430100086D2F3439272F302700",
            "804301000B6D2F3439272F30272F302700",
        ];
